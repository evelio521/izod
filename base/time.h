...
inline int64 GetTimeInMs() {
  ...
  return (int64)(now.tv_sec * 1000 + now.tv_usec / 1000);
}

inline int64 GetTimeInUs() {
  struct timeval now;
  gettimeofday(&now, NULL);
  return (int64)(now.tv_sec * 1000 * 1000 + now.tv_usec);
}

inline int64 GetTimeInNs() {
  struct timeval now;
  gettimeofday(&now, NULL);
  return (int64)(now.tv_sec * 1000 * 1000 * 1000 + now.tv_usec * 1000);
}

class BenchTimer {
 public:
  BenchTimer(const string& name, int repeat_times);
  ~BenchTimer();
...
inline string GetTimeOfDay(const Time& time) {
  Time::Exploded exploded;
  time.LocalExplode(&exploded);
  return StringPrintf("%.4d-%.2d-%.2d",
      exploded.year,
      exploded.month,
...
inline string GetDetailTime(const Time& time) {
  Time::Exploded exploded;
  time.LocalExplode(&exploded);
  return StringPrintf("%.4d-%.2d-%.2d %.2d:%.2d:%.2d",
      exploded.year,
      exploded.month,
      exploded.day_of_month,
      exploded.hour,
...
inline string GetUTCFormatTime(const Time& time) {
  Time::Exploded exploded;
